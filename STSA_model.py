...
class STSANet(nn.Module):
    def __init__(self):
        ...
        # Layer to replace MaxUnpool3d 
        self.unpool_2 = nn.Upsample(scale_factor=(1, 2, 2), mode='trilinear', align_corners=False)
        self.unpool_conv = nn.Sequential(
    ...
    def forward(self, x):
        ...
        add = self.unpool_conv(torch.cat((out1_0, out1_1, out1_2, out1_3), dim=2))
        ...
        add = self.unpool_conv(torch.cat((out1_01, out1_23), dim=2))
